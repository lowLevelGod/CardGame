        ...
        for (int horz = 0; horz < GRID_HEIGHT; horz++) {
            ...
            for (int vert = 0; vert < GRID_WIDTH; vert++) {
                if (horz != MID)
                {
                    ...
                    {
                        int tempy = y;
                        for (String line : troop.getPrintString().split("\n"))
                            g2d.drawString(line, x, tempy += g.getFontMetrics().getHeight());
                        // g2d.drawString(troop.getPrintString(), x + sizew / 4, y + sizeh / 4);
                    }
                    else
